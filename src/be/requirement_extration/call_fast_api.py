...
try:
    ...
    logger.warning(f"Blob Storage init failed: {e}")
    blob_service = None
    container_name = None
    container_client = None
...
async def compare_requirements_v0(name: str, compareToName: str):
    ...
    try:
        # Load the prompt message (use absolute path relative to this file)
        prompt_path = Path(__file__).parent / "prompt_message.txt"
...
def get_blob_content(blob_name: str) -> bytes:
    blob_path = f"out/{blob_name}"
    return bsc.get_blob_client(blob_path).download_blob().readall()
...
async def get_extracted_requirements_excel(name: str):
    ...
    # Blob-first: serve direttamente da Blob Storage (nessuna scrittura su ./out_flat o ./tmp)
    ...
    # Fallback: filesystem/cache o download da blob su disco (comportamento storico)
...
async def download_requirements_excel_by_query(
    ...
    if not filename:
        raise HTTPException(status_code=400, detail="Parametro filename obbligatorio")
...
async def extract_requirements(file: UploadFile = File(...)):
    ...
    try:
        ...
        if USE_NEW_ANALYZER:
            ...
            # Build graph-ready payload for Neo4j import.
            graph_payload = build_neo4j_graph_payload(
            ...
            # Save graph-ready JSON locally
            with open(
                out_flat_analisi_dir / f"{file_hash}_graph.json",
                "w",
                encoding="utf-8",
            ) as file:
            ...
            # Upload graph-ready version to blob as {hash}_graph.json
            graph_file_path = out_flat_analisi_dir / f"{file_hash}_graph.json"
            upload_to_blob(
                "requirements",
                file_path=graph_file_path,
                blob_name=f"{file_hash}_graph.json",
            )
            logger.info(
                f"{log_prefix}Uploaded graph JSON to blob storage: {file_hash}_graph.json"
            )
            ...
            # Copy and upload plain-named graph JSON
            plain_graph_path = out_flat_analisi_dir / f"{base_name}_graph.json"
            shutil.copy(
                out_flat_analisi_dir / f"{file_hash}_graph.json",
                plain_graph_path,
            )
            upload_to_blob(
                "requirements",
                file_path=plain_graph_path,
                blob_name=f"{base_name}_graph.json",
            )
...
async def download_comparison_excel(
    ...
    # Blob-first: serve direttamente da Blob Storage (nessuna scrittura su ./output)
    ...
    # Fallback: comparisonMapping (nome alternativo in blob)
    if pdf_json_mapping.comparisonMapping:
        comp_key1 = f"{plain1}_{plain2}"
        comp_key2 = f"{plain2}_{plain1}"
        for comp_key in (comp_key1, comp_key2):
            comp_file = pdf_json_mapping.comparisonMapping.get(comp_key)
            if not comp_file:
                continue
            excel_blob_name = (
                comp_file.replace(".json", ".xlsx")
    ...
    # Fallback: filesystem/cache (comportamento storico)
...
def get_hash_for_name(pdf_name: str) -> str:
    ...
    if raw.lower().endswith(".pdf"):
        raw = raw[:-4].strip()
    normalized = raw
    # Chiave canonica: solo lettere e numeri minuscoli (ignora spazi, underscore, trattini, punti)
    def canonical(s: str) -> str:
        return re.sub(r"[^a-z0-9]", "", s.lower())

    # Varianti da provare: nome esatto, solo nome file (senza path), e match per suffisso nei mapping
    keys_to_try = [
        normalized,
    ...
    # Match per suffisso: se il mapping ha "AML_INT_1_2_PRC_..." e abbiamo "INT_1_2_PRC_...", usa quello
    for map_key, mapped in pdf_json_mapping.mapping.items():
        if map_key == normalized or map_key.endswith(normalized) or normalized.endswith(map_key):
            return Path(mapped).stem
    # Match canonico: stesso nome ignorando punteggiatura/spazi/underscore (es. Policy-PCG... vs Policy_PCG...)
    ...
    if norm_canonical:
        for map_key, mapped in pdf_json_mapping.mapping.items():
            if canonical(map_key) == norm_canonical:
                ...
                return Path(mapped).stem
        # Ultimo tentativo: match canonico contiene o è contenuto (es. nome abbreviato)
        for map_key, mapped in pdf_json_mapping.mapping.items():
